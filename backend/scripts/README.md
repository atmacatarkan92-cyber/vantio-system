...
| `onboard_new_org.py` | ACTIVE | Create new organization + optional admin (CLI; dry-run unless `--apply`). | **WITH REVIEW** | Idempotent by `organization.slug` (Alembic 062+). Uses app `DATABASE_URL` and sets RLS org context before user/credential inserts. ⚠️ WRITES TO DATABASE |
