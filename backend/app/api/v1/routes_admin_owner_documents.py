"""
Admin owner documents: upload to R2, list metadata (org-scoped).
Mirrors routes_admin_landlord_documents for Eigentümer (owners).
"""
...
from db.models import Owner, OwnerDocument, User
...
router = APIRouter(prefix="/api/admin", tags=["admin-owner-documents"])
...
def _doc_to_dict(d: OwnerDocument, uploaded_by_name: Optional[str] = None) -> dict[str, Any]:
    return {
        "id": str(d.id),
...
def _resolve_object_key(doc: OwnerDocument) -> Optional[str]:
...
def _get_document_for_org(session: Session, document_id: str, org_id: str) -> OwnerDocument:
    doc = session.get(OwnerDocument, document_id)
    if not doc or str(getattr(doc, "organization_id", "")) != org_id:
        raise HTTPException(status_code=404, detail="Document not found")
    return doc


@router.get("/owner-documents")
def admin_list_owner_documents(
    ...
    rows: List[OwnerDocument] = list(
        session.exec(
            select(OwnerDocument)
            .where(OwnerDocument.owner_id == owner_id)
    ...
    def name_for(doc: OwnerDocument) -> str:
...
@router.get("/owner-documents/{document_id}/download")
def admin_owner_document_download(
    document_id: str,
    org_id: str = Depends(get_current_organization),
    _=Depends(require_roles("admin", "manager")),
    session: Session = Depends(get_db_session),
...
@router.post("/owner-documents")
@limiter.limit("60/minute")
def admin_create_owner_document(
    request: Request,
    owner_id: str = Form(...),
    ...
    try:
        key = build_owner_object_key(owner_id, raw_name)
