...
def calculate_monthly_revenue_for_units(session, unit_ids: list[str], year: int, month: int) -> dict[str, dict]:
    """
    Batch version of calculate_monthly_revenue to avoid N+1 queries when listing units.
    Returns { unit_id: { unit_id, year, month, expected_revenue, occupied_rooms, vacant_rooms, total_rooms } }.

    Uses the same revenue source of truth as KPI/profit: TenancyRevenue rows prorated by overlap days.
    """
    if not unit_ids:
        return {}
    ...
    rooms = list(
        session.exec(
            select(Room)
            .where(Room.unit_id.in_(unit_ids))
    ...
    # --- Overlapping tenancies for the month ---
    overlapping_tenancies = list(
        session.exec(
            select(Tenancy)
            ...
            .where(Tenancy.move_in_date <= last)
            .where((Tenancy.move_out_date == None) | (Tenancy.move_out_date >= first))  # noqa: E711
        ).all()
    )
    ...
    for t in overlapping_tenancies:
        tenancies_by_room_id.setdefault(str(t.room_id), []).append(t)
        ten_ids.append(str(t.id))

    # --- Revenue rows for those tenancies ---
    revenue_rows = (
    ...
    out: dict[str, dict] = {}
    for uid in unit_ids:
        unit_rooms = rooms_by_unit.get(str(uid), [])
